# class AllApDataInterpreter is used to create docking statistics for each Action point in fleet
# class AllRobotInterpeter is used to create docking statistics for each active Robot in fleet
# class BaseDataInterpret include all shared functions with those classes


################# class AllApDataInterpreter ##########################
#
# class AllApDataInterpreter creates initial ap statistics dataframe. Each column of an dataframe, have attached
# list of words located between (). This list indicates, values of which columns from dataframe related with docking&undocking action, 
# should be used to when creating statistics for single AP. Rules of processing values from every chosen column are defined in single_robot_dataframe_fill.
# function.
# After creation of initial dataframe, an single row of dataframe from top of docking&undocking action actions is taken to processing.
# If stats for given AP exists, row representing AP is updated.
# If stats for given AP doesnt exists, row representing AP is created.
# This process continues in loop, after all AP data from all registered docking actions dataframe is processed.

###### CHANGE IF ######:
# - want to change columns defined between (), from which values for statistics will be used to update/create single AP stats
# IF CHANGING NAME BETWEN () REMEMBER TO DEFINE RULES FOR PROCESSING COLUMN VALUES IN single_robot_dataframe_fill
# example: 'ACTION_ERROR(DOCK_TRY, UNDOCK_INCORRECT, ACTION_ERROR) --> 'ACTION_ERROR(DOCK_TRY))'
INITIAL_AP_STATISTICS_DATA = {
    'AP_NAME()': ["initial"],
    'SUCC_RATE()': [0],
    'SUCCESS()': [0],
    'FAILURE()': [0],
...
# In order to acces class AllApDataInterpreter funcionality, call generate_all_robot_dataframe() function on its object.

# Order of tasks done by function:
# 1. Create initial INITIAL_AP_STATISTICS_DATA ap statistics dataframe.
# 2. Starts iterating through each row of docking&undocking action dataframe. Each row contains data about single AP
# 3. Using information in INITIAL_AP_STATISTICS_DATA, statisics for single AP are updated/created
# 4. After processing all rows of docking&undocking action dataframe, ap statistics dataframe is returned
...
class BaseDataInterpreter:
    ...
    def create_init_data_df(self):
        for column, values in INITIAL_AP_STATISTICS_DATA.items():
            if not isinstance(values, list):
                raise ValueError(f"The data for column '{column}' should be a list.")
        
        interpreted_ap_df = pd.DataFrame(INITIAL_AP_STATISTICS_DATA)
        ...
        try:
            ...
            if interpreted_ap_df.loc[0, 'AP_NAME()'] == "initial":
                ...
                if name_found == False:
                    ...
                    self.action_point_dataframe = pd.DataFrame(INITIAL_AP_STATISTICS_DATA, index=[0])
                    self.action_point_dataframe['AP_NAME()'] = unique_name
                    interpreted_ap_df = pd.concat([interpreted_ap_df, self.action_point_dataframe], ignore_index=True)
